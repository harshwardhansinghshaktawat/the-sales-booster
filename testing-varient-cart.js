class ProductGalleryElement extends HTMLElement {
        ...
        try {
            ...
            if (name === 'products-data') {
                ...
                for (var i = 0; i < keys.length; i++) {
                    this.settings[keys[i]] = data[keys[i]];
                }
            ...
            console.error("Parse error in " + name + ":", e);
        }
    }

    // ======================================================
    // VARIANT DATA RECEIVED FROM WIDGET
    // ======================================================
    onVariantDataReceived(data) {
        ...
        // Reset the "Add to Cart" button on the card
        ...
        if (!data.hasOptions || !data.options || data.options.length === 0) {
            // No options - find default variant or add without variant
            var defaultVariantId = null;
            if (data.variants && data.variants.length > 0) {
                defaultVariantId = data.variants[0].id;
            }
            this.dispatchAddToCart(pid, defaultVariantId, 1, null, data.manageVariants);
            return;
        }

        // Has options -> show variant selector modal
        this.showVariantModal(pid);
    }

    // ======================================================
    // CART STATUS FROM WIDGET
    // ======================================================
    onCartStatus(data) {
        var pid = data.productId;
        ...
        if (data.status === 'loading') {
            ...
            var errEl = this.querySelector('.modal-error');
            if (errEl) { errEl.textContent = data.message || 'Error adding to cart'; }
        }
    }

    // ======================================================
    // DISPATCH ADD-TO-CART EVENT TO WIDGET
    // ======================================================
    dispatchAddToCart(productId, variantId, quantity, selectedChoices, manageVariants) {
    ...
    showVariantModal(pid) {
        ...
        for (var oi = 0; oi < data.options.length; oi++) {
            ...
            if (opt.type === 'color') {
                for (var ci = 0; ci < opt.choices.length; ci++) {
                    var c = opt.choices[ci];
                    var bgColor = c.color || c.value || '#ccc';
                    choicesHtml += '<div class="color-swatch" data-opt-name="' + this.escHtml(opt.name) + '" data-choice-value="' + this.escHtml(c.value) + '" title="' + this.escHtml(c.description || c.value) + '" style="background-color:' + bgColor + ';"></div>';
                }
                optionsHtml += '<div class="option-group"><label class="option-label">' + this.escHtml(opt.name) + '</label><div class="color-swatches">' + choicesHtml + '</div></div>';
            } else {
                for (var ci2 = 0; ci2 < opt.choices.length; ci2++) {
                    var c2 = opt.choices[ci2];
                    choicesHtml += '<div class="choice-btn" data-opt-name="' + this.escHtml(opt.name) + '" data-choice-value="' + this.escHtml(c2.value) + '">' + this.escHtml(c2.description || c2.value) + '</div>';
                }
                optionsHtml += '<div class="option-group"><label class="option-label">' + this.escHtml(opt.name) + '</label><div class="choice-buttons">' + choicesHtml + '</div></div>';
            }
        }
        ...
        overlay.setAttribute('data-modal-for', pid);
        overlay.innerHTML =
            '<div class="variant-modal">' +
            '<button class="modal-close-btn">&times;</button>' +
            '<div class="modal-header">' +
            (product ? '<img class="modal-thumb" src="' + (product.imageUrl || '') + '" alt="' + this.escHtml(product.name) + '">' : '') +
            '<div class="modal-header-text">' +
            '<div class="modal-title">' + this.escHtml(product ? product.name : '') + '</div>' +
            '<div class="modal-price" id="modal-price-display">' + (product ? product.price : '') + '</div>' +
            '</div></div>' +
            ...
            '</div>' +
            '<button class="modal-add-btn" disabled type="button">Select Options</button>' +
            '</div>' +
            '</div>';
    ...
    setupModalEvents(overlay, pid, data) {
        ...
        var priceDisplay = overlay.querySelector('#modal-price-display');
        var errorDisplay = overlay.querySelector('.modal-error');
        ...
        overlay.addEventListener('click', function (e) {
            if (e.target === overlay) self.closeVariantModal();
        });
        ...
        // Option selection (color swatches and choice buttons)
        ...
        for (var i = 0; i < clickables.length; i++) {
            clickables[i].addEventListener('click', function () {
                ...
                // Store selection: key = option NAME (e.g. "Color"), value = choice VALUE (e.g. "Red")
                ...
                if (allSelected) {
                    ...
                    // Try to find matching variant to update price
                    var matchedVariant = self.findMatchingVariant(pid, data);
                    if (matchedVariant) {
                        var variantPrice = matchedVariant.formattedPrice || matchedVariant.price;
                        if (variantPrice && priceDisplay) {
                            priceDisplay.textContent = typeof variantPrice === 'number' ? '$' + variantPrice.toFixed(2) : variantPrice;
                        }
                    }
                }
        ...
        addBtn.addEventListener('click', function () {
            ...
            var selectedObj = self.selectedChoices[pid] || {};
            var matchedVariant = self.findMatchingVariant(pid, data);

            // Build selectedChoices as { "OptionName": "ChoiceValue" }
            // This is the format Wix expects for catalogReference.options
            self.dispatchAddToCart(
                pid,
                matchedVariant ? matchedVariant.id : null,
                quantity,
                selectedObj,
                data.manageVariants
            );
    ...
    // Find variant that matches all selected choices
    // Variant.choices format: { "Color": "Red", "Size": "Large" }
    findMatchingVariant(pid, data) {
        ...
        for (var i = 0; i < data.variants.length; i++) {
            ...
            var variantChoices = v.choices || {};
            var match = true;

            for (var k = 0; k < selectedKeys.length; k++) {
                var optName = selectedKeys[k];
                if (variantChoices[optName] !== selected[optName]) {
                    match = false;
                    break;
                }
            }
    ...
    renderProducts() {
        ...
        for (var i = 0; i < this.products.length; i++) {
            html += this.renderCard(this.products[i]);
        }
        grid.innerHTML = html;
        ...
        for (var b = 0; b < cartBtns.length; b++) {
            cartBtns[b].addEventListener('click', function (e) {
                ...
                this.disabled = true;
                // Dispatch request-variants to widget
    ...
    renderCard(product) {
        ...
        return '<div class="product-card">' +
            (product.ribbon ? '<div class="ribbon">' + this.escHtml(product.ribbon) + '</div>' : '') +
            '<div class="img-wrap">' +
            '<img src="' + (product.imageUrl || 'https://via.placeholder.com/400') + '" alt="' + this.escHtml(product.name) + '" loading="lazy" onerror="this.src=\'https://via.placeholder.com/400\'">' +
            '</div>' +
            '<div class="card-body">' +
            '<h3>' + this.escHtml(product.name) + '</h3>' +
