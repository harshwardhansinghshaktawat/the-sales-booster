class ProductGalleryElement extends HTMLElement {
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        if (newValue && newValue !== oldValue) {
            if (name === 'products-data') {
                console.log('CUSTOM ELEMENT - Products data attribute changed, length:', newValue.length);
                try {
                    const data = JSON.parse(newValue);
                    console.log('CUSTOM ELEMENT - Parsed products data:', data.products.length, 'products');
                    console.log('CUSTOM ELEMENT - First product received:', JSON.stringify(data.products[0], null, 2));
    ...
    renderProducts() {
        console.log('CUSTOM ELEMENT - Rendering products, count:', this.products.length);
    ...
    renderProductCard(product) {
        console.log('CUSTOM ELEMENT - Rendering card for product:', product.name, 'Price:', product.price);
        
        const hasComparePrice = product.compareAtPrice && product.compareAtPrice !== product.price;
        
        // Check if price is undefined, null, or empty
        const displayPrice = product.price || 'Price not available';
        
        console.log('CUSTOM ELEMENT - Display price:', displayPrice, 'Compare price:', product.compareAtPrice);
